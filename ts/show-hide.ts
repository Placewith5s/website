class Expand_Collapse {
    ...
    constructor() {
        ...
        this.#show_hide_btn = this.#main.querySelector(
            "#show-hide-btn"
        );

        if (this.#show_hide_btn) {
            this.#show_hide_btn.addEventListener(
                "click",
    ...
    #hide_elements_by_default(): void | never {
        const hidden_elements: NodeListOf<HTMLElement> =
    ...
    #check_hidden_elements(
        hidden_elements: NodeListOf<HTMLElement>
    ): void {
        ...
        try {
            ...
            hidden_elements.forEach((elem) => {
                ...
                const match: RegExpMatchArray | null =
                    elem.id.match(/step-hidden-from-(\d+)/);
            ...
            throw new Error(
                `Error during toggle of hidden steps: ${err}`
            );
        }
    }
    ...
    #toggle_elem_visibility(
        ...
        try {
            ...
            throw new Error(
                `Error occurred while toggling element visibility: ${err}`
            );
        }
    }

    #update_btn_text_and_aria(): void {
        if (this.#show_hide_btn) {
            if (
                this.#show_hide_btn.textContent ===
                "Show All"
            ) {
                this.#show_hide_btn.textContent =
                    "Hide All";
                this.#show_hide_btn.setAttribute(
                    "aria-expanded",
                    "true"
                );
            } else {
