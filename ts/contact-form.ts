class Character_Counter {
    ...
    constructor(text_area_element: HTMLTextAreaElement) {
        ...
        this.#char_count_element =
            this.#main.querySelector(".char-count");

        if (
            this.#text_area_element &&
            this.#char_count_element
        ) {
            this.#max_char_count =
                this.#text_area_element.getAttribute(
                    "maxlength"
                );
            this.#handle_invalid_char_count(0);
            this.#text_area_element?.parentNode?.appendChild(
                this.#char_count_element
            );
            ...
            this.#text_area_element.addEventListener(
                "input",
                this.#throttled_upd_char_count
            );
        } else {
            throw new Error(
                "Missing required Character Counter elements!"
            );
        }
    }
    ...
    #upd_char_count(): void {
        const current_length: number =
            this.#text_area_element?.value?.length;
        this.#handle_invalid_char_count(current_length);
    }
}

const message_input: HTMLTextAreaElement | null =
